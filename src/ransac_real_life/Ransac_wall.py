...
bp=[[-0.7,0.15],[2,2.5],[-0.7,-2.5],[2,-0.15]] 
# bp=[[-0.7,0.15],[2,0.8],[-0.7,-0.8],[2,-0.15]] 
...
def ransac(X, y, max_iters=70, samples_to_fit=2, inlier_threshold=0.17, min_inliers=30):  
...
def range_filter(msg):
    ...
    right_dist = [x for x in ranges[250:320] if x<2]
    left_dist=[x for x in ranges[40:110] if x<2]
    # right_dist = [x for x in ranges[250:320] if x<0.9]
    # left_dist=[x for x in ranges[40:110] if x<0.9]
...
def scan_callback(msg):
    ...
    if len(lines_m)>0:
        ...
        point = PointStamped()
        point.header.frame_id = 'base_link' # "base_footprint"
        point.point.x = p
