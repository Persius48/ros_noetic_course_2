...
# parameters
yaw_precision_ = (math.pi / 180)*10 # +/- 2 degree allowed
dist_precision_ = 0.1
# publishers
pub = None
right=[]
...
bp=[[0.5,0.100],[.8,0.2],[0.5,-0.2],[.8,-0.100],[0.15,0.1],[0.5,0.2],[0.15,-0.2],[0.5,-0.1]]
...
def clbk_laser(msg):
    ...
    right_dist = [x for x in custom_range[0:70] if x<2]
    left_dist = [x for x in custom_range[110:180] if x<2]
    ...
    regions = {
        ...
        'right-1': min(min(custom_range[20:70]),0.75),
        ...
        'left-1': min(min(custom_range[110:160]),0.75),
    }
...
def front_boxes(msg):
    ...
    print('number of points detected in left_1 box',X_left_1.shape[0])
    print('number of points detected in right_1 box',X_right_1.shape[0])
...
def go_straight_ahead(des_pos):
    ...
    if err_pos > dist_precision_:
        ...
        # if regions['right-1']< 0.3:
        #     while regions['right-1']<=0.35:
        #         twist_msg.angular.z = 0.2 
        #         # twist_msg.linear.x = 0.5
        #         print('close to the right-1')
        #         pub.publish(twist_msg)
        #         # if regions['front']>.8 and regions["left-1"]>0.65:
        #         #     break
        #     change_state(2)
        # elif regions['left-1']< 0.3: 
        #     while regions['left-1']<=0.35:
        #         twist_msg.angular.z = -0.2 
        #         # twist_msg.linear.x = 0.5
        #         print('close to the left-1')
        #         pub.publish(twist_msg)
        #         # if regions['front']>.8 and regions["right-1"]>0.65:
        #         #     break
        #     change_state(2)
