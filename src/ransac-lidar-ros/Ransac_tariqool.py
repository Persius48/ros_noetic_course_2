...
def dist_avg_callback(msg):
    global right_dist_avg,left_dist_avg

    right_dist_avg = msg.data[0]
    left_dist_avg = msg.data[1]
...
def ransac(X, y, max_iters=70, samples_to_fit=2, inlier_threshold=0.17, min_inliers=6):  
...
def scan_callback(msg):
    ...
    for j in range(0,len(bp),2):
        ...
        if (X.shape[0]<=2) or (y.shape[0]<=2):
            ...
            if result is None:
                continue
            X_all.append(X)
            # print('number of points = ',len(X_all))
            y_all.append(y)
            # print('number of points = ',len(y_all))
            # print('result = ',result)
            m = result[0][0]
            # print('slope: ',m)
            b = result[1][0]
    ...
    # rectangle4 = Rectangle(bp[6], width4, height4, linewidth=1, edgecolor='g', facecolor='none')
    ax = plt.gca()
    ...
    elif len(lines_m) == 0 and position_.x > 0.5 and position_.x < 9.5:
        p = 0.5
        if right_dist_avg > left_dist_avg: 
            new_y = -0.3
        elif right_dist_avg < left_dist_avg:
            new_y = 0.3
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('dist_avg', Float32MultiArray, dist_avg_callback)
