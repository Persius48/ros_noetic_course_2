...
state_ = 0
# parameters
yaw_precision_ = (math.pi /180)*5 # +/- 2 degree allowed
bp=[[0.5,0.100],[1.3,0.27],[0.5,-0.27],[1.3,-0.100]]#,[0.15,0.1],[0.5,0.2],[0.15,-0.2],[0.5,-0.1]]
...
def rotate (angular_speed_degree, relative_angle_degree, clockwise):
    ...
    yaw_offset=math.degrees(yaw_imu)

    while True :
        yaw=math.degrees(yaw_imu)
...
def setDesiredOrientation(speed_in_degree, desired_angle_degree):
    global yaw_imu
    relative_angle_radians = math.radians(desired_angle_degree) - yaw_imu
    clockwise=0
...
def go_straight_ahead(des_pos):
    ...
    if err_pos > dist_precision_:
        twist_msg = Twist()
        twist_msg.linear.x =0.5  #0.10 + .25*err_pos
        if err_yaw > 0.06:
                twist_msg.angular.z = 0.1 
        elif err_yaw< -0.06:
            twist_msg.angular.z = -0.1 
        ...
        for i in range(3):
            ...
            while not rospy.is_shutdown():
                if state_ == 0:
                    ...
                    if i ==0:
                        setDesiredOrientation(23,desired_orientation_)
                    if i ==1:
                        setDesiredOrientation(23,90+desired_orientation_)
                    if i ==2:
                        setDesiredOrientation(23,180+desired_orientation_)
                    change_state(1)
                    # print(i)
                    # print('state is 0')
                elif state_ == 1:
