...
bp=[[-0.7,0.15],[2,0.85],[-0.7,-0.85],[2,-0.15]] 
...
def ransac(X, y, max_iters=50, samples_to_fit=2, inlier_threshold=0.25, min_inliers=6):  
...
def range_filter(msg):
    global right_dist,right_dist_avg, left_dist,left_dist_avg,regions
    row_width = 0.8
    ranges=msg.ranges
...
def scan_callback(msg):
    ...
    if len(lines_m)==2:
        ...
        if right_dist_avg <left_dist_avg:
            ...
            mid_point_y= -0.2
            p=0.75
        elif right_dist_avg==left_dist_avg==0:
            mid_point_y=0
            p=.25
...
if __name__ == '__main__':
    ...
    sub_laser_1 = rospy.Subscriber('/scan', LaserScan, scan_callback)
    sub_laser_2 = rospy.Subscriber('/scan', LaserScan, range_filter)
