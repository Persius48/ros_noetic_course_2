...
bp=[[0.5,0.100],[1.5,0.27],[0.5,-0.27],[1.5,-0.100]]#,[0.15,0.1],[0.5,0.2],[0.15,-0.2],[0.5,-0.1]]
...
def clbk_odom(msg):
    ...
    if n==0:
        ...
        if position_.x<=left_x_higher_limit:
            active_= True
        elif position_.x>left_x_higher_limit:
            active_= False
    # yaw
    quaternion = (
...
#     print('yaw_imu =',math.degrees(yaw_imu))    

def rotate (angular_speed_degree, relative_angle_degree, clockwise):
    global pub,yaw_,case_,point_
...
def go_straight_ahead(des_pos):
    ...
    # print("desired yaw : ",math.degrees(desired_yaw))
    # print('current yaw : ',math.degrees(yaw_))
    err_yaw =normalize_angle(desired_yaw - yaw_)
...
def main():
    global pub, position_,active_,desired_orientation_,switch,row_width,left_x_higher_limit,left_x_lower_limit,desired_position
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(3):
            ...
            if i==2 :
                ...
                desired_position.x = left_x_higher_limit+0.1
            ...
            while not rospy.is_shutdown():
                if state_ == 0:
                    ...
                    if i ==0:
                        change_point(i)
                        setDesiredOrientation(15,desired_orientation_) #30
                    if i ==1:
                        change_point(i)
                        setDesiredOrientation(15,desired_orientation_-90)
                        # rotate(15,90,clockwise=True)
                    if i ==2:
                        change_point(i)
                        setDesiredOrientation(15,desired_orientation_-180)
                        # rotate(15,90,clockwise=True)
                    done(0)
                    change_state(1)
                    # print(i)
